...
export default function TechnicalCalendarPage({ token, notify, serviceOrders = [], devices = [], segments = [], groups = [], permissions = {}, focusServiceOrder, onFocusHandled }) {
  ...
  const [filters, setFilters] = useState({ technicianId: "", eventType: "", status: "", priority: "", search: "" });
  ...
  return <section className="technical-calendar-page">
    ...
    <div className="calendar-filters"><label><Search size={16} /><input value={filters.search} onChange={(e) => setFilters((current) => ({ ...current, search: e.target.value }))} placeholder="Buscar título, OS ou técnico" /></label><select aria-label="Filtrar por técnico" value={filters.technicianId} onChange={(e) => setFilters((current) => ({ ...current, technicianId: e.target.value }))}><option value="">Todos os técnicos</option>{technicians.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Filtrar por tipo" value={filters.eventType} onChange={(e) => setFilters((current) => ({ ...current, eventType: e.target.value }))}><option value="">Todos os tipos</option>{Object.entries(EVENT_TYPE_META).map(([id, meta]) => <option key={id} value={id}>{meta.label}</option>)}</select><select aria-label="Filtrar por status" value={filters.status} onChange={(e) => setFilters((current) => ({ ...current, status: e.target.value }))}><option value="">Todos os status</option>{Object.entries(EVENT_STATUS_LABELS).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select><button type="button" className="icon-button" onClick={load} title="Atualizar"><RefreshCw size={16} /></button></div>
