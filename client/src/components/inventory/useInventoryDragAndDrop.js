...
export function useInventoryDragAndDrop({ devices, filteredDevices, segments, onMoveMachine }) {
  ...
  const handleDragEnd = useCallback((event) => {
    ...
    if (overMachineId && machineId !== overMachineId) {
      setOrderedIds((current) => {
    ...
    if (machine.segmentId === targetSegmentId) {
      return { machine, targetSegmentId, targetType, moved: false };
    }

    onMoveMachine(machine, targetSegmentId);
    return { machine, targetSegmentId, targetType, moved: true };
  }, [devices, onMoveMachine]);
